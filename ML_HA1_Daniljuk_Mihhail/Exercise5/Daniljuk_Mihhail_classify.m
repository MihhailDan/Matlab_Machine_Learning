...
function [value, DS, lbls] = Daniljuk_Mihhail_classify(DS_test, labels_test, dec_tree)
    ...
    for j = dec_tree(:,1)'
        if ith_DS(i,j) > dec_tree(count,2)
            if dec_tree(count,3) ~= 0
                DS_test(i,4) = dec_tree(count,3);
                break
            end
        else
            if dec_tree(count,4) ~= 0
                DS_test(i,4) = dec_tree(count,4);
                break
            end
        end
...
value = length(find(DS_test(:,3) == DS_test(:,4)));
DS = DS_test;
lbls = DS_test(:,4);
end